...
| 指数の早朝更新 | `scripts/us_index_update.sh` | 毎日 06:30 | 日経・S&P500の指数終値＋セクター別インパルス（下落メーター用） |
| 米国株ランキング | `scripts/us_ranking_update.sh` | 毎日 07:00 | S&P500の時価総額・出来高ランキング（yfinance） |

**なぜ 6:30 と 7:00 に分けるか**: 下落上等ページの下落メーターは「暴落の朝に規模を
すぐ知る」ためのもの。US引け（16:00 ET = JST 夏5:00/冬6:00）後、できるだけ早く
更新したい。6:30 JST は冬でも 16:30 ET でクローズ確定後になる**通年最速の固定時刻**。
一方、約500銘柄を取るランキングは引け直後の未確定値リスクを避けて 7:00 のまま。
指数・インパルスは 7:00 側でも重複実行される（差分同期のため無害。6:30失敗時の保険）。
**6:00 ちょうどは不可**: 冬は 16:00 ET ジャストで未確定バーガード（16:05 ET）に
弾かれ、前日分しか入らず1日遅れになる。
...
### 2. スクリプトに実行権限を付与
```bash
chmod +x scripts/daily_update.sh scripts/us_ranking_update.sh scripts/us_index_update.sh
```
...
| サーバーTZ | 夜バッチ(JST21:10) | 指数早朝(JST06:30) | 米国株(JST07:00) |
|---|---|---|---|
| **Asia/Tokyo (JST)** | `10 21 * * 1-5` | `30 6 * * *` | `0 7 * * *` |
| **UTC** | `10 12 * * 1-5` | `30 21 * * *` | `0 22 * * *` |
| その他 | JSTから各自換算（JST = UTC+9） | 同左 | 同左 |

> UTC換算の考え方: JST 21:10 = UTC 12:10、JST 06:30 = UTC 21:30（前日扱い）、
> JST 07:00 = UTC 22:00（前日扱い）。
...
# 市場指数の早朝更新（毎朝06:30 JST。US引け直後 = 冬16:30/夏17:30 ET）
...
30 21 * * *   /path/to/web_analize/scripts/us_index_update.sh
...
- `scripts/us_index_update.sh` … 指数の早朝更新。日経・S&P500の指数終値
  （下落上等ページの下落メーター用）とセクター別インパルスの日次終値を取る。
  軽い2コマンドのみ（計1分弱）。暴落の朝にメーターを最速で当日値にするのが目的。
  日曜のみ指数を `--full` で全期間取り直す（Yahoo側で消えて後から復活した日の
  穴埋め。保存済みは ignore_conflicts で捨てるため既存データは壊れない。数秒）。
  曜日判定は内蔵しているので cron は毎日の1行でよい。
- `scripts/us_ranking_update.sh` … 米国株ランキング専用。S&P500構成銘柄(約500)の
  ...
  日曜のみ発行済株式数を取り直す。指数・インパルス・マクロ指標もここで再実行される
  （6:30側の保険）。
