...
- 月次ページ「月内の使い方」の点線は**平常月の推移**＝直近12か月の「日別累計」を日ごとに
  平均した曲線を月初から月末まで予め引く（`SpendingSetting.monthly_target` があれば形は
  そのまま月末を目標額に縮尺）。⚠️ **直線（月合計÷日数）にしないこと**。家賃・カード
  引き落としで月内の使い方は毎月同じ形に波打つので、直線では今日時点の速い/遅いが
  読めない（実際に「オレンジの直線では指標にならない」と指摘された）。
  ⚠️ **当月の実績（緑）は今日より先を描かない**（`None`）。描くと今日の値のまま月末まで
  横一線に伸び「線が機能していない」ように見える（これも実際に指摘された）
