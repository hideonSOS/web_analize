...
**横軸=直近20営業日、縦軸=独自セクター**の時系列ヒートマップ。日々の騰落状態を
並べてモメンタムの継続・転換を見る（1日分のヒートマップならTradingViewで足りる、
というユーザー方針で時系列型にした）。ナビ・トップページの機能6はこちら。

- **独自セクター定義は `japan_kabu/impulse.py` の `IMPULSE_SECTORS`**（プロトタイプ
  段階のため各セクター数銘柄限定）。銘柄・セクターの追加はこのファイルを編集して
...
- **`codes` が空のセクションは「ダミー行」**として擬似データで描画する（画面に
  ...
  **現在ダミー行は無い**（US全8行とも構成銘柄が定義済み）。ただし
  US既存3行=AI・半導体/ビッグテック/金融 の銘柄は暫定であり差し替え待ち。
- **現物・先物ではなく関連株/ETFで代替している行がある**（`_targets()` が Stock マスタ
  経由で解決するため、`BTC-USD`・`GC=F` 等の非株式ティッカーはそのままでは使えない）:
  暗号資産=MSTR/COIN（Strategy・Coinbase）、金銀=GLD/SLV（ETF）。
  非株式ティッカーを使いたくなったら `_targets()` にマスタを介さない経路の追加が必要。
- 宇宙系 = **SPCX**(Space Exploration Technologies=SpaceX本体・**2026-06-12上場**)
  /SIDU/MNTS/SATL。SPCXは上場が新しく履歴が短いため、σ推定できる日数
  （`MIN_HISTORY`）を割ると `NEUTRAL_BAND` にフォールバックする点に注意。
- セルは構成銘柄の日次騰落率の**単純平均**を3値分類（灰=休場・データ無し）。
- **⚠️ 判定しきい値は固定%ではなく「そのセクター自身のσの±0.5倍」**（`SIGMA_BAND`）。
...
- データは `update_impulse_prices`（yfinance・JP/USとも）が **DailyPrice に蓄積**。
  差分同期。初回は `--days 60`。**cron は `us_ranking_update.sh` に同梱済み**（JST朝7時。
...
- セクター名の下の構成銘柄は**先頭3件＋「他N」に省略**（`_members_label`）。
  全部並べるとセクター列が横に伸びてヒートマップ本体を押し出す。全銘柄名は title 属性で見られる。
