...
- **セクター名の下の構成銘柄ラベルは2026-08-31に廃止した**（ユーザー要望: 銘柄は
  必要なときだけ見られれば十分。ドローダウンの内訳とセクター名のツールチップ
  =判定幅＋構成銘柄一覧で確認できる）。空いたぶんセルを30→32pxに拡大し、
  impulse.css 内で `.mc` を max-width:1150px に上書きしている（インパルス専用CSS
  なので他ページに影響しない）。`_members_label` はドローダウンページで使用中。
  **ラベルを復活させないこと**（セクター列が横に伸びてヒートマップ本体を押し出す）。
