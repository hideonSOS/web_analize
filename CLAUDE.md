...
カルテ詳細の株価セクションに POST ボタン（`karte:fetch_prices` → `call_command('update_daily_prices',
code=..., years=3)` を同期実行）を置いた。1銘柄・差分同期なので数秒（gunicorn の既定タイムアウト
30秒内）。結果は messages で株価セクションに出す（base.html は messages を描かないので detail.html
側で描く）。⚠️ `update_daily_prices` の **JP 銘柄は J-Quants 無料プランで直近が取れない**ため、
失敗か0件なら **yfinance（`<コード>.T`）に自動で切り替える**ようにした（実測: ソニーで J-Quants 0件 →
yfinance +6日）。全銘柄一括はこのボタンからは回さない（バッチの仕事）。
...
- ⚠️ **業績推移は 2026-09-16 に大幅更新**（ユーザー指摘「直近の決算が含まれない通期だけの棒は表示する価値が
  著しく低い／前年同期比も比べられない」）: **既定は四半期（直近8期・単独値）、通期はタブ**。どちらも
  グラフ下の表に **前年同期比（売上・営業利益・純利益）と営業利益率**（`_trend_pack`。前年同期は end の差が
  350〜380日の期を探す。前年が 0 以下なら '—'）。JP の四半期は累計なので同一年度内で差し引いて単独値に
  （`_jp_single_quarters`・4Q=FY−3Q）。**ラベルは期間そのもの**（`_period_label`: '26/4–6'、'25/4–26/3'）。
  「2026年」は 2025/4〜2026/3 を 2026年と誤読させ、「2026/3期」も PayPay の呼称 FY2025 と1年ズレて見えた。
  年や期末年月の表記に戻さないこと
- ⚠️ **分割調整（2026-09-16 に発見・修正）**: yfinance の株価は `auto_adjust=False` でも**分割は常に調整済み**。
  一方 EDGAR の株式数・EPS・1株配当は提出時点のままなので、分割の2年以上前の期の PER/PBR が 10〜20 倍
  ズレていた（NVDA 2021/10 の PER 7.9 倍＝実際は約 79 倍、GOOG/AMZN は 1/20）。後年の提出で再表示された
  値は分割後なので、**「期末が分割前」かつ「提出日も分割前」のものだけ** yfinance の分割履歴で調整する
  （`edgar.split_factor`・`_latest_by_period` が filed を返す）。現在値は分割後の四半期なので影響なし。
  日本株は J-Quants の株価が調整済みで株式数も開示どおりだが、引き継ぎ中の配当が分割前のまま残る
  （ソニー 5:1 後の利回り 3%）ので、株式数の跳ね（1.5倍超/0.67倍未満）を分割とみなして割る（`_build_history`）
...
TTMの計算は `japan_kabu/indicators.py` の `_ttm_np`（日本株）と `_ttm_np_us`（米国株）で
分岐している。**混同すると利益が4倍や1/4になるので注意。**

### 指標の定義（2026-09-16 に Yahoo・決算短信と突合して揃えた。変えるなら再突合すること）
ユーザー要望「株探等と著しく乖離していないか。でたらめだと大きなダメージ」。突合の結果と対処:
- **PER・PBR は米国株で Yahoo と一致**（AMZN 19.8/20.0、NVDA 26.7/26.8、GOOG 17.1/17.1、PBR も同じ）。
  TSLA だけ 370 vs 330（Yahoo は希薄化後 EPS、こちらは発行済株式数ベース）
- **日本株の ROE/ROA の分母は「自己資本」（＝EqAR×総資産）**。J-Quants の `Eq` は純資産で非支配持分を
  含むため、そのまま割ると ROE が過小（住友林業で 13%）。`_jp_own_equity`（2026-09-16）
- **ROE は 2〜4割低く出ていた**（NVDA 84 vs 117、GOOG 38 vs 49）。原因は分母: こちらは期末自己資本、
  Yahoo と決算短信の「自己資本当期純利益率」は **期首期末平均**。→ 平均に統一（`_avg`・`_rep_year_before`）。
  日本株の利益も通期実績から **TTM**（1Q 開示後に更新）に変更。予想 EPS が無い会社（NEC・SBG）は
  PER を実績 TTM で出す（`per_basis`。ラベル「PER（実績）」）
- **ROA は Yahoo と定義が違う**（Yahoo は税引後営業利益÷平均総資産で、純利益ベースの約半分に出る）。
  こちらは純利益÷平均総資産（決算短信・株探と同じ）。Yahoo と突合しないこと
- ⚠️ **ソニー 2026/3 期の純利益 −3,268 億円は本物**（金融分社化に伴う包括利益の振替損）。ROE が
  マイナスに出るのはデータ誤りではない。継続事業ベースで見たいなら Yahoo（13%）と食い違って当然
...
- **ドル円と政策金利（2026-09-08 追加）**: ドル円は FRED `EXJPUS`（月中平均・円/ドル）で日米両タブに
  同じチャート（`chart-fx`・`unit: '円'`。macro.js は `spec.unit` で単位と `scale` を切替）。
  米の政策金利は既存の `FEDFUNDS`（チップ追加）。**日本の政策金利は無担保コール翌日物
  `IRSTCI01JPM156N`（月平均・約2か月遅れ）で代用**。⚠️ 中銀金利そのものの `IRSTCB01JPM156N` は
  2023-12、公定歩合 `INTDSRJPM193N` は 2017-04 で配信停止（実測）。DBnomics の BIS 政策金利は
  データセット名が見つからず不採用
- **「特殊」タブ（`?country=SP`・2026-09-08）= 標準化して重ねる相関分析**（`japan_kabu/macro_special.py`）。
  ...
  **カルテ登録銘柄も選べる**（キー `K:<code>`・`karte_catalog()`・DailyPrice の月末終値を
  `_karte_monthly` で月次化。履歴は DailyPrice の約3年ぶんなので、期間を古くしても銘柄側は
  2023-07 からしか描けない）。ページ下部に**相関ヒートマップ**（`heatmap()`・`?h=<key>` で基準を
  1つ選ぶ）: 行=他の全系列（選択期間の |r| 順）、列=選択期間/直近5年/3年/1年、色は赤=正・青=負・
  濃さ=|r|（`_heat_color`）。サーバー側で色まで決めて描く（JSなし）。12か月未満は計算しない、
  36か月未満は薄字。⚠️ 3年しか無い銘柄は「選択期間」「直近5年」「直近3年」が同じ値になる（正常）
...
- 一覧(`/karte/`)は **カードが上・指標の比較表が下**（2026-09-16 ユーザー指示）。カードは ⠿ を
  ドラッグで並び替えでき、`StockKarte.sort_order`（0014）に保存（`karte:reorder_cards`・「推しを前に」
  の用途。updated_at は動かさない）。既定の並びは sort_order → 更新日の新しい順
- 一覧(`/karte/`)の**押し目一覧は 2026-09-16 に削除**（指標の比較表に1年ドローダウン列があり重複・
  ユーザー指示）。比較表の母集団はカルテ銘柄のみ。戻さないこと
...
- **旧「銘柄別指標」ページ（全銘柄 3,704 件・5.8MB を一括埋め込み・生成 3.8 秒）は 2026-09-16 に廃止し、
  ...
  - **時価総額・発行済株式数**（2026-09-16）: 直近開示の株式数（PER と同じ。米国株は分割調整済み）× 現在株価。
    詳細の指標見出し行と一覧の比較表に出す（`fmt_shares` / `fmt_money`。米国株は円換算を併記）。
    ⚠️ ランキングの `Stock.market_cap`（S&P500・yfinance 株式数）とは根拠が違うので混ぜない
  - カルテ一覧の **「指標の比較」表**: ユーザー決定の5列 = PER／PBR／ROE／配当利回り／1年ドローダウン＋時価総額。
    押し目が深い順。「—」は決算未取得
  ...
  - **米国株は円換算を併記**（2026-09-16 ユーザー要望「ドル表記はそのままに円換算も併記、Y軸は円」）:
    `indicators.py` が `fx`（`portfolio.FxRate` の最新ドル円）を添え、`karte_indicators.js` が
...
## 📓 売買日記の振り返り（2026-09-22 から複数回・追記式）
`DiaryReview`（entry FK・result・note・created_at）。「振り返りを追記」で1件ずつ増え、カードに古い順で全部出す
（× で1件削除）。`DiaryEntry.review_*` は最新の写し（一覧表の評価列）。既存の振り返りは 0018 で1件目として取り込み済み。
上書き式に戻さないこと（ユーザー指示「何度も積み重ねて保存」）

## 📓 売買日記の損益表示（2026-09-21）
- **買い**: 「概算損益」＝（現在値 − 購入価格）× 株数（含み損益）。「その後 +x%」
- **売り**: **「実現損益」＝（売値 − 買値）× 株数**。買値は **同じ銘柄の売りより前の直近の買いの記録**
  （`index` の `buys_by_stock`。同一銘柄で売買を繰り返す使い方なので直近の買いで実態に合う、がユーザー決定）。
  買いの記録が無ければ「買値の記録なし」。売却後の値動きは「売却後 +x%」の%だけ残す。
  ⚠️ 旧「概算損益」（売りは「売却後の値下がり＝回避額」）は利確でもマイナスに見えて誤解を招いたので売りでは出さない

## 📓 売買日記の「売り」フォーム（2026-09-19 に一新・ユーザー指示。買いは別途見直す予定）

売りで記録するのは **①分類（利益確定／損切り）②ルールどおりにできたか（できた／できなかった）** の2点と
自由記述だけ（`DiaryEntry.sell_kind` / `rule_followed`・0016）。**判断理由タグ・心理は買い・売りとも廃止**
（同日ユーザー指示。フォームから削除し保存もしない。`tags`/`mood` 列は過去の記録の表示のため残す。
`TAGS`/`MOODS` 定数は練習ページが使う）。「想定内/外」も売りでは出さない（`exit_expected` は練習だけ）。
短期トレードの決済との連動: 損切り → `exit_reason='stop'`、利益確定 → 価格から 利確／早期利確 を判定。
利益確定なのに損切り線以下で売った場合は裁量（manual）として残す。カードには分類バッジと
「ルールどおり ✔／ルール逸脱 ✖」を出す。**心理は不要**というのが本人の決定なので戻さないこと

## 🎯 短期トレード（`/diary/contra/`）— 2026-09-09 追加

**画面上の名前は「短期トレード」**（順張りもするので「逆張り」から改名。ユーザー決定）。
コードの識別子は `contra` のまま（URL・`Trade.strategy='contra'`・`ContraSetting`・
`diary/contra.py`）。UI 文言に「逆張り」を戻さないこと。
**ルールは追跡対象にした時点で固定**（ユーザー決定）: 日記の「短期トレードとして追跡する」を
チェックすると、目標・損切りは設定の既定（利確 +10%／損切り −5%）で埋まりロックされる（JS）。
サーバー側 `open_from_entry` も日記の価格を無視して既定%で線を引き、日記の出口計画を同じ価格に
上書きする（二重の縛り。フォームを迂回しても崩れない）。ルールを変えるのは設定画面だけ。

米国株で「大幅安の日に乗り、損切り・利確を先に決めて機械的に運用する」短期トレードの帳簿と
成績ページ。**売買日記の下位機能**（ユーザー決定 2026-09-09）: ナビには出さず、日記の
「逆張りの結果追跡 →」ボタンから遷移する。**入力は売買日記に統一**し、逆張りページは結果の
追跡と成績だけ（エントリー／決済フォームは置かない。戻さないこと）。
- 日記の「買い」に **「逆張りで結果を追跡する」チェック**（フォーム内・一覧の各行）→
  `contra.open_from_entry` が `Trade` を起こす（損切り/利確は日記の価格から%を逆算。無ければ
  設定の既定%）。一覧のチェックを外すと `untrack`（未決済のみ。日記の行は残る）
- 日記の「売り」→ 同じ銘柄の保有中の取引を `close_from_entry` で決済。理由はフォームの
  ラジオ（未選択なら約定価格から 損切り線以下=損切り／利確線以上=利確／それ以外=裁量）
- 日記の行を消すと: エントリー側なら取引ごと消す、決済側なら取引を未決済に戻す（`delete`）
- 実装は `diary/models.py`（`ContraSetting` / `Trade` / `TradeBar`）、`diary/contra.py`（計算と
  日記連動）、`diary/views.py`（`create` / `track` / `contra`）、`update_trade_bars`

### ルール（ユーザーと合意済み。変えるときは本人の決定で）
- 損切りは経費。経費として成立する条件は「勝率 > 分岐勝率」と「1回の損失が資金の1〜2%以内」
- 分岐勝率 = 損切り幅 ÷（損切り幅＋利確幅）。コスト込みは 勝ち＝利確−2c・負け＝損切り＋2c
  （c=片道0.5%）。既定 5%/10% → 名目33%・コスト込み40%。**数字は §4 のバックテスト（未実施）
  で確定させる予定**（急落後の +10%/+15% 到達率と、安値から反転までの最大DD）
- **判定はザラ場**（日足の安値/高値）。指値を入れる運用なので終値は合わない（ユーザー決定）。
  だから `TradeBar` は **生の OHLC（auto_adjust=False）** を保有中の取引だけ毎日取る
  （`update_trade_bars`。朝の `us_index_update.sh` と夜の `daily_update.sh` に同梱。画面の
  「日足を更新」ボタンでも同期実行）。⚠️ 調整後終値の `DailyPrice` は使わない
- **資金は手入力**（`ContraSetting.capital`。遊び・学習目的。ポートフォリオと連動させない、
  ユーザー決定）。許容株数 = 資金×リスク% ÷（価格×損切り幅）。超えて入ると
  `over_risk`（裁量）として記録し画面に出す
- **為替は考えない**（ユーザー決定 2026-09-09）。一度ドルに替えたら円に戻さず運用するので、
  資金・損益・リスクはすべて取引の通貨（ドル）のまま。`FxRate` は使わない（`latest_fx` は残置・未使用）。
  ⚠️ 円換算を足さないこと（戦略の精度だけを見たい、が目的）
- 片道コストは **0.132%**（moomoo証券ベーシックコースの米国株手数料 0.12%・税込 0.132%・
  上限 $22。2026-09-09 実測・ユーザー指定）。既定 5%/10% → コスト込みの分岐勝率 **35%**。
  上限 $22 は未考慮（$16,700 超の注文でだけ効く）。為替は引き続き考えない
- 「今回は損切りを見送る」は裁量介入（`exit_reason='manual'`）。回数と損益を成績に出して監視
- **勝率はルール決済（利確・損切り）だけで数える**（ユーザー決定 2026-09-09）。+10% に届く前に
  +5〜6% で降りた「早期利確」（`exit_reason='early'`。売りの価格が建値より上で利確線未満なら
  自動判定）は、+10% 前提の分岐勝率と競合するので勝ち負けの数に入れない。裁量・期限も同じ。
  ただし損益は**トータルに積算**する（成績の「早期利確 n件 +x%」と内訳に出す）。
  経費判定の最低件数（10件）もルール決済だけで数える
- ⚠️ **成績（勝率・円グラフ・トータル損益・経費判定・振り返り）は決済済み（`exit_date` あり）だけ**で
  集計する（ユーザー確認 2026-09-09）。保有中の含み損益は「保有中」カードの表示だけで、成績には
  絶対に混ぜないこと

### 画面
- 保有中: 1件1ブロック。**レンジバー**（左端=損切り線・右端=利確線・▲建値・●現在値）、
  経過日数、現在価格（最新日足の終値）、損切り／利確までの%、含み損益、期間の安値/高値。
  **安値が損切り線／高値が利確線に触れた取引は枠を赤/緑にして先頭に上げ、決済フォームに
  約定価格と理由を先埋め**する（指値が約定しているはず、という前提）
- 新規エントリー: 銘柄検索（日記と同じ JSON）→ 価格自動 → 損切り線・利確線・分岐勝率・
  許容株数を JS で即時表示。「N株を入れる」で埋まる
- 成績: 勝率（勝ち＝コスト込みで損益プラス）vs 分岐勝率、期待値、裁量回数、最大連敗、
  累積損益の階段グラフ、決済一覧。勝率が分岐勝率を上回れば緑
- 設定は畳んだ `<details>`（月次支出と同じ流儀）

### 未実施（次にやること）
- §4 バックテスト: S&P500 銘柄の日足数年分を一度取り、「終値でX%以上下げた日」を起点に
  最大DD・+7.5/10/15% 到達率・到達日数を集計するコマンドと結果ページ。損切り幅の根拠になる
- 「大幅安の日」の候補一覧（`Stock.change_pct` が毎日入っているので作れる）

## 🎮 練習（`/diary/practice/`）— 2026-09-10 追加

投資家の先輩直伝の練習法: **買ったつもりで実際には賭けず、ルールで追う**。ナビ「記録」の「練習」。
短期トレードと**同じレイアウト**（勝率の円グラフ・保有中のレンジバー・振り返りのタブ・
画面下の固定ルールバーに「練習」バッジ）だが、**売買日記とは一切つながない**（ユーザー決定）。
- データは同じ `Trade` テーブルで `strategy='practice'`、設定は `ContraSetting.get('practice')`
  （`kind` 列で短期と分離。軍資金・ルール%・コストを別に持てる）。日足は `update_trade_bars` が
  戦略を問わず保有中を全部取るので追加作業なし
- 入力はこのページのフォームで直接（銘柄検索・価格・株数・**なぜ買ったか（必須）**・タグ・心理）。
  タグ・心理は日記が無いので `PracticeMeta`（Trade に1対1）に持ち、振り返りはそこから読む
  （`contra.stats._reflect_row` が `practice_meta` を見る）。決済もこのページ（価格から理由を自動判定）
- 短期トレード側の集計は `strategy='contra'` で絞っているので練習は混ざらない。
  `open_rows` / `stats` は `strategy` 引数で切り替える。⚠️ 新しい集計を足すときも必ず戦略で絞ること
- JS は `contra.js`（円グラフ・振り返りタブ。要素IDが同じなので共用）＋ `practice.js`（銘柄検索と
  許容株数）。CSS は `contra.css` を共用
- **保有中の1銘柄ブロックは `diary/_position.html` を両ページで include**（2026-09-10）。見た目を
  同じに保つための共通化で、片方だけ直さないこと（`close_hint` だけページごとに渡す）。
  レンジバーには 損切り／−2.5／0（建値）／+2.5／+5／+7.5／利確 のスリットとラベル（`contra._ticks`。
  主目盛りと副目盛り）、**現在値はシアンのスリット＋バー上の ▼＋騰落%**（旧 ● は廃止）、メタに「利確まで達成 x%」
  （＋なら利確幅に対する進み、−なら「損切りまで進行 x%」）を出す（ユーザー要望: 半分で降りるか・
  達成率を見たい）。ルールバーは練習側に「練習」バッジを付けて取り違えを防ぐ
- 練習ページの並びは 成績 → **買ったつもりで記録** → 保有中 → 振り返り → 設定（保有が増えても
  入力フォームが上にあるように。ユーザー要望 2026-09-10）。売却は各玉の目立つボタン
  「売却を記録する」で開き、**復習コメントは必須**（損切り・利確のたびに「なぜこうなったか・次に
  どうするか」を書く。本人いわく「自分を知る最良の教科書」）。振り返り一覧には判断理由の下に
  復習コメントを全文で出す（`.ct-reflect-review`。短期トレード側も同じ表示で、そちらは売りの
  日記の判断理由が復習コメントになる）
- **学習用の3要素（2026-09-10・両ページ共通）**: ①なぜ買ったかは「＋ 理由を追加」で**1行1理由に列挙**
  （`Trade.entry_note` に改行区切り。`contra.reasons_of` で分解。日記経由は日記の判断理由の行）
  ②**マイナスになる想定**（`Trade.risk_scenario`。練習は必須、日記は追跡チェック時に任意）と、
  決済時の**想定内／想定外（寝耳に水）**（`Trade.exit_expected`。練習の売却フォームと日記の売りの
  ラジオ）。損切りが予測できていたかを振り返りで突き合わせる ③**保有中のコメント追記**
  （`TradeNote`。追記のみ・編集しない。「下がってショック」等の当時の心理を時系列で残す。
  各ページの `form_id='note'`）。振り返りの1件は `diary/_reflect_item.html` を両ページで include
  （理由の列挙 → 想定＋想定内/外 → コメントの時系列 → 復習）。片方だけ直さないこと
- コメントの種別は **好材料（緑）／悪材料（赤）／情報（白）**（`TradeNote.kind`。旧 `mood` は未使用）。
  **購入時の理由と購入後のコメントは区別せず同じ時系列**（`contra.timeline`。ユーザー方針「同じ土俵で
  評価したい」）。購入時の理由は `TradeNote(at_entry=True, created_at=建て日 9:00)` として入り、
  左端に日付ではなく「購入時」と出る。表示順は 記入日時（日時 or 購入時）→ 材料の種類 → コメント。
  練習の入力フォームは理由1行ごとに種別を選べる（`reasons` / `reason_kinds`）。日記経由は判断理由の
  各行を「情報」で入れる。既存データは 0014 のデータ移行で `entry_note` の行から取り込み済み。
  並びは `-at_entry, created_at`（購入時が常に先頭。建て日より前の時刻に書いたコメントがあっても崩れない）
- **チャートのスクリーンショット**（`TradeNote.image`・2026-09-10）: 貼り付け欄（`.ct-paste`）か同じ
  フォームのテキスト欄で **Ctrl+V** → `contra.js` が canvas で 1200px 幅・JPEG 0.85 に縮小 → data URL を
  hidden に入れて送信 → DB にそのまま保存（上限 `MAX_IMAGE_CHARS`）。表示は `diary:note_image`
  （data URL を復号して返す。ログイン必須）。⚠️ `/media/` は nginx が認証なしで配信するので使わない。
  練習の入力フォーム（購入時のチャート）と、両ページのコメント追記フォームで貼れる
- ⚠️ **実データ（短期・contra）と練習（practice）を混ぜないこと**（ユーザーが繰り返し強調）。
  取引を触る POST は必ず `get_object_or_404(Trade, pk=..., strategy=...)` でページの戦略に縛る
  （短期ページから練習の取引にコメントを付けると 404 になることをテストで確認済み）。集計・一覧は
  `strategy` で絞る。共通なのは表示部品（include・CSS・JS）と日足取得だけ
- **売却後の追跡**（`diary/_after_exit.html`・`contra.after_exit_rows`）: 損切り・利確に関わらず、売った後の
  値動き（現在値・売却後の高値/安値・売却価格比）と一言（「損切り後に反発、切り所が早かったか」等）を
  ページ最下部に出す。`update_trade_bars` は**売却後 `AFTER_EXIT_DAYS`（30日）まで日足を取り続ける**
- **損切り・利確シミュレーター**（`diary/_simulator.html`・2026-09-14・両ページの成績の直下に include）:
  本人の使い方は「逆張りで『ここまでは落ちない』ラインの近くで買うとき、**買値を入れると一瞬で上下のライン
  （損切り線・利確線の価格）が見える**」。買値だけで動き、3枚の大きな数字（損切りライン／買値／利確ライン）と
  保有中と同じ目盛りバー（−5／−2.5／0／+2.5／+5／+7.5／+10 の価格）を出す。任意入力: **株数**があれば金額
  （損切りしたら／利確したら・赤緑の棒・軍資金×リスク%の損失上限ゲージ）、**サポート価格**があれば損切り線との
  上下関係と「サポートを損切り線に合わせるなら買値は $x 以下」（= 損切りに合わない買い場の上限）。
  ルール値はページの設定を data 属性で渡す。計算は `contra.js` のみ（コストは往復 2c。`contra.breakeven` と
  同じ定義）。DB には触れず、最後の入力を localStorage に覚えるだけ（`sim_key` で短期と練習を別キーに）。
  両ページで include するので片方だけ直さないこと
- **円換算の併記とタイムリミット**（2026-09-24 ユーザー要望・両ページ共通）: 米国株の現在値（バー上の吹き出し `$225.51（¥35,229）+6.6%`。**右上の見出しは購入価格からの1株あたり増減 `現在 +$14.51（+¥2,293）` の2つだけ**・2026-09-24 ユーザー指示）・含み損益・トータル・決済一覧に
  最新ドル円（`latest_fx`＝portfolio.FxRate）で円を併記（表示だけ。ルール・勝率・分岐勝率の判定には使わない。
  「為替は考えない」方針は維持）。**タイムリミット**は価格のレンジバーと同じ grid（`.ct-range`）の2段目に細い線（左右の端がバーと一致・
  現在位置は線の下の ▲・価格バーと区別）: **20日で満了・15日（3/4 の位置）に白い印**（同日 30→20日に短縮・ユーザー指示。`TIME_LIMIT_DAYS`/`TIME_WARN_DAYS`・`time_gauge`）。青→15日で琥珀→20日で赤「満了」。
  満了で自動決済はしない（本人が判断）
- **建値ストップ**（2026-09-24 ユーザー採用・両ページ共通）: 高値が建値 +7% に届いたら、損切りの逆指値を建値へ上げる
  （利確 +10% はそのまま）。根拠はユーザー銘柄15・10年の検証（−5%急落の日に入る・14営業日）で 1回あたり期待値
  +7%全利確 −0.20% ／ +10%全利確 +0.11% ／ **+7%到達で建値へ +0.15%**（+7%到達の83%はそのまま+10%へ、14%は
  +7%→−5% の往復だった＝実感の「7%で調整」の正体）。`ContraSetting.be_trigger_pct`（既定 7・0 で無効・設定画面）を
  追跡開始時に `Trade.be_trigger_pct` へ固定。`contra.breakeven_state` が wait／reached（高値が届いた＝琥珀の案内と
  「変更した」ボタン）／moved（`Trade.be_moved_at`・損切り表示が建値に変わる・取り消し可）を返す。バーに 🛡 の印、
  ローソク足に琥珀の点線。建値へ上げた後の決済は `auto_exit_reason` が建値付近（+1%以内）を **'breakeven'＝建値撤退**
  と判定（勝率の分母には入れない・損益はトータルに積算・内訳に「🛡 建値撤退」）。自動で注文は変えない（本人が証券会社で変更）
- **取得日からのローソク足**（2026-09-24・両ページ共通）: 価格のレンジバーと期限の線の間（同じ `.ct-range` grid の中段）。
  横軸は期限の線と同じ（左端=取得日・右端=`TIME_LIMIT_DAYS`・縦の点線=15日）、縦軸は損切り〜利確（はみ出した
  日があれば広げる）。水平線 緑=利確／白=建値／赤=損切り。`contra.candles` がサーバー側で SVG 座標まで作る
  （JS なし・`TradeBar` の生 OHLC）。期限を過ぎた日は描かない。日足が無ければ「日足待ち」。高さは 288px（同日 96px→3倍・ユーザー指示。スマホは 200px）
- ⚠️ 建てた当日は日足が無く現在値マーカーが出ない（実例: GOOG）。`open_rows` は日足が無いとき
  株価マスタの終値（`Stock.close`）で代用し「日足待ち」と表示する。日足が来れば自動で切り替わる
...
- ⚠️ **API は手入力の記録しか返さない**（口座連携の自動取込は返らない、と公式注記）。実測
  （2026-09-06 `--check`）: 手入力＋レシートの行は 1 件残らず一致、欠けるのは 楽天カード連携
  1,137 行・三菱UFJ 連携 371 行・給料 76 行・balance 67 行。そこで **`fetch_zaim` は最新の手動 CSV
  から口座連携の行だけを補完して1本の CSV にする**（`_manual_supplement`。判定は「方法が balance」
  または「(支払元, 入金先) の組が API 側に無い」。API が返す組の行は取らないので重複しない）。
  結果、分析期間の台帳は手動 CSV と**完全一致**（7列キーで差 0）した上で直近月だけ新しくなる。
  ⚠️ 補完元の手動 CSV が古いと連携行もその分古い。**月1回程度は手動アップロードを続ける**
  （カード分は e-navi、家賃・給料は `ufj_bank/` が持つので、忘れても集計は壊れない）
- ⚠️ エクスポート CSV の癖に合わせている（`_csv_quirk`）: お店が空なら `-`、先頭が `-` の文字列には
  `'` が付く。合わせないと同じ記録が別 `ledger_id` になり手動修正が外れる（実測 3 行）
- 認可（`scripts/zaim_authorize.py`）で踏んだ罠: ①`oauth_callback='oob'` は 401（URL 形式必須）
  ②トークン取得は **POST**（GET は 401 Unauthorized）③署名の正規化パラメータを二重エンコードすると
  日付だけの GET では通るのに callback 付きで 401 になる ④端末で折り返された認可 URL をコピーすると
  トークンが壊れ「メンテナンス中か端末の設定」画面になる（ブラウザ自動起動にした）
  ⑤Zaim は verifier 無しでもアクセストークンを返す ⑥トークンを画面に出すと端末・会話に残る
  （config.json へ直接書く方式にした）
- **「Zaim だけで足りるか」の検証結果（2026-09-06・実データ）**: 楽天カードは Zaim の連携行と
  e-navi CSV が分析期間で**件数・金額・最新日まで完全一致**（156 行・¥460,595・2026-09-03）なので
  e-navi は代替可能。**三菱UFJ は不可**: Zaim の銀行連携は **2025-04-29 で止まっている**
  （payment 最終 2025-04-29・給料 2025-04-25。分析期間 2025-08〜 に 0 行）。家賃・光熱費・給料・
  引き落としカレンダーは `ufj_bank/` の銀行 CSV が唯一の供給源。**銀行 CSV を外さないこと**
- **ソースの鮮度と「揃っている月」（`spending/freshness.py`・2026-09-06）**: Zaim だけ毎晩自動で、
  カード・銀行・Amazon は手動のまま、という前提でレイアウトと判定が壊れないようにしてある。
  `source_freshness()` がソースごとの最終日（cash/card/bank は Transaction の max(date)、Amazon は
  order_date）と古さ（`STALE_DAYS`: cash 4日・card/bank 35日・Amazon 100日）を出し、
  **全ソースが揃っている最終月 `full_through_ym`** を決める。使い方:
  ①トップに鮮度テーブル（古いものは赤＋何をアップするか）②月次ページに「この月のデータ:
  現金 6日まで／カード 3日まで／銀行 2日まで」の揃い具合（揃えば自動で消える）
  ③平常月（`monthly.build` の baseline_months）・記録の質（`_record_quality` の partial）・
  サブスクの連続判定（`_monthly_profile(cutoff_ym=カードの最終月)`）は `full_through_ym` までを
  根拠にする。⚠️ これを外すと、カード CSV が2か月古いだけで全サブスクが「止まっている」に落ち、
  未設定率が偽って跳ね、平常月の中央値が下がる（シミュレーションで確認済み）。
  ファイルをアップロードすれば最終日が進んで注意書きは自動で消える
...
- 月次ページの**初期表示は当月**（2026-09-22 ユーザー指示。以前は「当月は数日分で前月比が読めない」として直近の完了月を出していたが、当月の進み具合を見るのが主用途）。当月のデータが無ければ最新月
- 月次ページ「月内の使い方」は**ユーザーの明示指示（2026-09-06）で形が確定**している:
  線＝直近12か月それぞれの「日別累計」を日ごとに平均した曲線（家賃・カード引き落としの
  日で段が付く右肩上がり。下がらない。`SpendingSetting.monthly_target` があれば形は
  そのまま月末を目標額に縮尺）、棒＝今月の支出の累積。**両方とも累積で同じスケール1つ**。
  棒が線を越えた日は赤。当月の未来日は棒を描かない（`None`）。
  ⚠️ 却下済みの形に戻さないこと: ①日別の棒＋累計線＋右軸 ②日別の棒＋累計の直線ペース
  ③日別の棒＋日ごとの目安曲線 ④日別の棒＋平均支出額の水平線
  ⑤累積の棒＋直線（1日平均×日数）（「引き落としのある日は増加するやろ」で却下）
