...
- **損切り・利確シミュレーター**（`diary/_simulator.html`・2026-09-14・両ページの成績の直下に include）:
  投資金額を入れるだけで、そのページの設定（損切り%／利確%／片道コスト・data 属性で渡す）から
  「損切りしたら −$x（残り）／利確したら +$y（合計）」を3枚のボックスと赤緑の棒（幅は金額比）で出し、
  利確1回で損切り何回分か、軍資金×リスク%の損失上限に収まるか（ゲージ・超えると赤＋上限額）を添える。
  計算は `contra.js` のみ（コストは往復 2c。`contra.breakeven` と同じ定義）。DB には触れず、最後の
  入力を localStorage に覚えるだけ（`sim_key` で短期と練習を別キーに）。両ページで include するので片方だけ直さないこと
