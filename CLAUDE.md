...
- 月次ページ「月内の使い方」の点線は**目標ペース**（`SpendingSetting.monthly_target` を
  その月の日数で日割り。未設定なら平常月＝直近12か月の中央値を代用）。「予めこの線を
  越えない」ための上限線。⚠️ **当月の累計は今日より先を描かない**（`None`）。描くと
  今日の値のまま月末まで横一線に伸び「線が機能していない」ように見える（実際に指摘された）
