...
- 月次ページ「月内の使い方」は**縦軸=金額・横軸=日の1スケールだけ**。棒=今月その日に
  使った額、点線=直近12か月の「その日に使った額」の平均を3日移動平均で均した目安線を
  月末まで予め引く（`SpendingSetting.monthly_target` があれば形はそのまま月合計を
  目標額に縮尺）。棒が点線を越えた日は赤。
  ⚠️ **累計線・右軸を足さないこと**（日別と累計はスケールが2桁違い「異なるスケールを
  混ぜていて意味が分からない」と指摘された）。⚠️ **直線（月合計÷日数）にしないこと**
  （家賃・カード引き落としの日が必ず越えて指標にならない、これも指摘された）。
  当月の未来日は棒を描かない（`None`）
