...
成績ページ。**売買日記（判断の記録）とは別に `Trade`（往復の帳簿）を持つ**が、エントリー／
決済のたびに日記へ「買い」「売り」を自動で書く（`strategy='contra'`・`trade` FK）ので、
日記は「全部の記録」として読める。実装は `diary/models.py`（`ContraSetting` / `Trade` /
`TradeBar`）、`diary/contra.py`（計算）、`diary/views.py contra`、`update_trade_bars`。
