...
レシート撮影は手入力扱いなので取れる。手動 CSV には 楽天カード連携 1,138 行・
三菱UFJ連携 391 行があり（2026-09-04 の実物）、これらが API で欠ける可能性がある。
`--check` は最新の手動 CSV と **方法×支払元** で件数を比べ、欠けを表で出す。

| 結果 | 対処 |
|---|---|
| 欠けなし | 手動アップロードは不要。cron 任せでよい |
| 楽天カード連携が欠ける | 実害は小さい（カード明細は e-navi が持ち、突合で埋まる）。ただし Zaim 側で付けた分類は失われ「推定」になる |
| 銀行連携が欠ける | 家賃・給料は三菱UFJ の CSV（`ufj_bank/`）が持つので実害なし |
| 直近月の件数が極端に少ない | 権限・`--since` を疑う。`fetch_zaim` の出力の「記録 N件」を見る |

欠けが大きい場合は、月1回程度の手動アップロードを併用する（新しい方が採用されるだけなので、
API 生成ファイルと手動ファイルが混在しても壊れない）。
...
- 署名は OAuth 1.0a HMAC-SHA1 を標準ライブラリで自前実装（依存追加なし）。
  requests-oauthlib と同じ署名になることを確認済み
